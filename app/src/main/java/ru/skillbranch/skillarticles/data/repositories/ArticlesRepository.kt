...
import ru.skillbranch.skillarticles.data.remote.res.toCategory
...
object ArticlesRepository : IArticlesRepository {
    private var network = NetworkManager.api
    ...
    override fun rawQueryArticles(filter: ArticleFilter): DataSource.Factory<Int, ArticleItem> {
        ...
        return articlesDao.findArticlesByRaw(SimpleSQLiteQuery(query))
    }
...
class ArticleFilter(
    ...
    fun toQuery(): String {
        ...
        if (categories.isNotEmpty()) qb.appendWhere("category_id IN (${categories.joinToString("\",\"", prefix = "\"", postfix = "\"")})")
        qb.orderBy("date")
        return qb.build()
    }
}
